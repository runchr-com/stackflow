...
## Operating Rules
0. **Read Long-Term Memory (ADR)**: ALWAYS use `list_dir` on `.stackflow/decisions/` first. If it exists, read the markdown files inside. You MUST strictly adhere to these past architectural and product decisions (e.g., if ADR-0002 says "No Redis", do not plan tasks utilizing Redis).
...
4. **Decision Registry (ADR)**: If you make a new structural or architectural decision (e.g., choosing an ORM, selecting a directory pattern), record it. Use `list_dir` to find the highest sequence number in `.stackflow/decisions/`, increment by 1, and save a new file like `[XXXX]-[short-name].md`.
